...
)
{
  ...
  FileList.FindHardLink (EnumDestination, 1, &aStats, &PathNameStatusList, NULL);
...
)
{
  ...
  CloneList.FindHardLink (CloneDestination, 0, &CloneStatistics, &ClonePathNameStatusList, NULL);
...
//    bool admin = IsProtectedFolder(L"c:\\windows");
    ...
    if (SmartMove)
    {
      ...
      if (INVALID_FILE_ATTRIBUTES != Argv1Path.FileAttribute)
      {
        ...
        dg = FileList.SmartMove(&aStats, &PathNameStatusList, NULL);
//        BOOL b = MoveFileEx(Argv1, Argv2,  MOVEFILE_DELAY_UNTIL_REBOOT);
    ...
    if (SmartRename)
    {
      ...
      int dg = FileList.SmartMove(&aStats, &PathNameStatusList, NULL);
